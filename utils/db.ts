...
export async function createUser(user: User, session: string) {
  const usersKey = ["users", user.id];
  const usersBySessionKey = ["users_by_session", session];
  const newUser: User = { ...user, session };

  const atomicOp = kv.atomic()
    .check({ key: usersKey, versionstamp: null })
    .check({ key: usersBySessionKey, versionstamp: null })
    .set(usersKey, newUser)
    .set(usersBySessionKey, newUser);
...
export async function updateUser(user: User) {
  const usersKey = ["users", user.id];
  const usersBySessionKey = ["users_by_session", user.session!];
...
export async function updateUserSession(user: User, session: string) {
  const userKey = ["users", user.id];
  const oldUserBySessionKey = ["users_by_session", user.session!];
  const newUserBySessionKey = ["users_by_session", session];
  const newUser: User = { ...user, session };
...
export async function getUserBySession(session: string) {
  const key = ["users_by_session", session];
